...
namespace Youtube2Spotify.Helpers
{
    ...
    public static class YoutubePlaylistItemFactory
    {
        ...
        public static YoutubePlaylistItem GetYoutubePlaylistItem(string song, string artist)
        {
            ...
            youtubePlaylistItem.originalName = song;
            string cleanedSongName = string.Join(' ', song.ToLower().Split(' ').Select(x =>
            {
                if (x.Contains("[official") || x.Contains("(official") || x.Contains("audio)") || x.Contains("video)") || x.Contains("audio]") || x.Contains("video]") || x.Length <= 1)
                {
                    return string.Empty;
                }
                return x;
            ...
            artist = string.Join(' ', artist.ToLower().Split(' ').Select(x => 
            { 
                if (x.Length <= 1) 
                { 
                    return string.Empty;
            ...
            youtubePlaylistItem.artist = artist;
